...
class CA():
    ...
    def __init__(self, master, globalVar): # 增加一个全局变量接收参数globalVar = scale_rate
        ...
        self.label1 = ttk.Label(self.label1_frame, text="尺寸校准", justify="left")
        ...
        # Z=4
        self.label2_frame = ttk.Frame(self.mainframe, width=184, height=21)
        self.label2 = ttk.Label(self.label2_frame, text="测量10mm的标准长度", justify="left")
        self.label2.place(x=0, y=0, width=184, height=21)
        self.label2_font = tkFont.Font(family="微软雅黑", size=11, weight="normal", slant="roman", underline=False)
        self.label2.configure(font=self.label2_font)
        self.label2_frame.place(x=372, y=4)
        # Z=2
        ...
        self.button1 = ttk.Button(self.button1_frame, text="校  准", command=self.detect)
        ...
        self.hotkey.register(('f10',), callback=self.set_xy)
    ...
    def set_xy(self, *args):
        ...
        if self.draw:
            d = ((self.x - x) ** 2 + (self.y - y) ** 2) ** 0.5
            self.label2.configure(text=f'Scale : { int(d*10000)/100000 } px / mm')
            self.close()
            self.globalVar.set(d/10)
        self.x = x
        self.y = y
